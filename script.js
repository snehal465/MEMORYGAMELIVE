...
function checkMatch(){
    ...
    secondCard.querySelector('.card-back').textContent
     ...
     console.log(firstCard.querySelector('.card-back').textContent,secondCard.querySelector('.card-back').textContent
);
    ...
    if(isMatch){
        ...
        matchedPairs++;
        //TODO
        console.log('max score' ,parseInt(symbols.length/2))
        console.log('score' , matchedPairs)

        let maxScore = parseInt(symbols.length/2);
        if(maxScore == matchedPairs)
        {
               statusText.textContent == 'you found all pairs!🥳️';
